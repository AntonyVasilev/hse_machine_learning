...
def prepare_loaded_dataframe(
    ...
    cleaned_torque_df = pd.DataFrame(
        cleaned_torque.to_list(), columns=["torque", "max_torque_rpm"]
    )
    ...
    df_prepared = pd.concat(
        [
            df_prepared,
            pd.DataFrame(
                data=polynominal_features.transform(df_prepared[poly_cols]),
                columns=polynominal_features.get_feature_names_out(),
            ).drop(poly_cols + ["1"], axis=1),
        ],
        axis=1,
    )
    encoded_cols_test = pd.DataFrame(
        data=one_hot_encoder.fit_transform(df_prepared[ohe_cols]).toarray(),
        columns=one_hot_encoder.get_feature_names_out(),
    )
    df_prepared = pd.concat(
        [df_prepared.drop(ohe_cols, axis=1), encoded_cols_test], axis=1
    )
    ...
    df_prepared = pd.DataFrame(
        data=min_max_scaler.transform(df_prepared), columns=df_prepared.columns
    )
...
def parce_2_part(value: str) -> float:
    ...
    if "+" in value:
        ...
        splitted_val_2 = convert_to_float_torque(
            splitted_values[1].split("rpm")[0].strip().split("-")[1].strip()
        )
        max_torque_rpm = splitted_val_1 + splitted_val_2
    elif "-" in value:
        max_torque_rpm = max(
            [
                convert_to_float_torque(val.strip())
        ...
        max_torque_rpm = convert_to_float_torque(value.lower().split("rpm")[0].strip())
    ...
    try:
        ...
        if "@" in value:
            ...
            if "nm" in value.lower():
                return convert_to_float_torque(
                    value.lower().split("nm")[0].strip()
                ), None
            else:
        ...
        if "nm" in value_1.lower():
            ...
            torque = (
                convert_to_float_torque(value_1.lower().split("kgm")[0].strip())
                * 9.80665
            )
        elif "(" in value_1:
