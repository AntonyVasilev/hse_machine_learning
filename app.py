...
try:
    model, feature_names = load_model(
        model_path=MODEL_PATH, feacture_names_path=FEATURE_NAMES_PATH
    )
...
try:
    min_max_scaler, one_hot_encoder, ordinal_encoder, polynominal_features = (
        load_transforms(path=MODEL_DIR)
    )
    (
        cols_with_na,
        na_fill_mapping,
    ...
    ) = load_additional_data(path=DATA_DIR)
except Exception as e:
...
df_to_plot = pd.DataFrame(
    data=pd.concat([pd.Series(feature_names), pd.Series(weights)], axis=1)
)
df_to_plot.columns = ["feature_name", "weight"]

if chkbx:
    intercept_df = pd.DataFrame(
        [{"feature_name": "intercept", "weight": model.intercept_}]
    )
    df_to_plot = pd.concat([df_to_plot, intercept_df], ignore_index=True)

st.line_chart(
    df_to_plot, x="feature_name", y="weight", x_label="Weight", y_label="Feature names"
)
...
with plot_col1:
    ...
    n_cars_by_brand = (
        df_to_plot.groupby("brand")
        .agg(n_cars=("name", "count"))
        .reset_index()
        .sort_values("n_cars")
    )
    n_cars_by_brand["n_cars_part"] = (
        n_cars_by_brand["n_cars"] / n_cars_by_brand["n_cars"].sum()
    )
...
with plot_col4:
    ...
    mean_prices_by_year = (
        df_to_plot.groupby(["year", "seats"])
        .agg(mean_price=(target_col_name, "mean"))
        .reset_index()
    )
            ...
            for col in main_columns:
                if features_base_dtypes[col] in ("object", "bool"):
                    ...
                    input_data[col] = st.selectbox(
                        col, values_for_form[col], key=f"cat_{col}"
                    )
            ...
            for col in main_columns:
                if features_base_dtypes[col] not in ("object", "bool"):
                    ...
                    input_data[col] = st.number_input(
                        col, value=values_for_form[col], key=f"num_{col}"
                    )
